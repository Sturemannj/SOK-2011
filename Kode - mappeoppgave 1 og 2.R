...
library(dplyr)
avg_educ_by_region <- df_growth2019 %>%
  group_by(region) %>%
  summarise(mean_educ = mean(avg_educ, na.rm = TRUE))

library(ggplot2)
educ_plot <- ggplot(avg_educ_by_region, aes(x = region, y = mean_educ, fill = region)) +
  ...
  guides(fill = FALSE) +
  geom_text(aes(label = round(mean_educ, 2)), vjust = -0.5, size = 4)
